...
public partial class RecoverPage : ContentPage
{
    ...
    public RecoverPage(ILocalizationResourceManager resourceManager)
    {
    ...
    private void CheckVeriftyCode()
    {
        ...
        AjaxMsgModel ajaxMsg = APIHelper.Query($"/api/user/CheckPhoneVerifyCode", dicParameters);
        if (ajaxMsg != null && ajaxMsg.Status.Equals("Success", StringComparison.OrdinalIgnoreCase))
        {
            slFirstStep.IsVisible = false;
    ...
    private void btnRecover_Clicked(object sender, EventArgs e)
    {
        ...
        AjaxMsgModel ajaxMsg = APIHelper.Query($"/api/user/recover", dicParameters);
        if (ajaxMsg != null && ajaxMsg.Status.Equals("Success", StringComparison.OrdinalIgnoreCase))
        {
